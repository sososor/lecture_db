...
class _RagCache:
    def __init__(self, embeddings: np.ndarray, texts: list[str], metas: list[dict]):
...
def _build_rag_cache_from_rows(rows: list[tuple]) -> _RagCache | None:
    ...
    metas: list[dict] = []
    embeddings: list[np.ndarray] = []
    for _idx, content, metadata, embedding in rows:
        if not content:
            continue
    ...
    return _RagCache(mat, texts, metas)
...
def _load_rag_cache_from_db() -> _RagCache | None:
    with _db() as con:
        cur = con.execute(
            """
            SELECT idx, content, metadata, embedding::float4[]
            FROM rag_chunks
            ORDER BY idx ASC
            """
        )
...
def _ingest_rag_vectors() -> _RagCache | None:
    ...
    with _db() as con:
        ...
        insert_sql = (
            "INSERT INTO rag_chunks(idx, content, metadata, embedding) VALUES (%s, %s, %s, %s::vector)"
            if use_vector
            else "INSERT INTO rag_chunks(idx, content, metadata, embedding) VALUES (%s, %s, %s, %s)"
        )
        if RAG_FORCE_RELOAD:
        ...
        for i, (emb, text, meta) in enumerate(zip(embeddings, texts, metas)):
            ...
            batch.append((idx, text, Json(inserted_metas[-1]), emb_val))
            if len(batch) >= 200:
    ...
    if RAG_CACHE_IN_MEMORY:
        ...
        mat = _normalize_matrix(mat)
        return _RagCache(mat, inserted_texts, inserted_metas)
    return None
...
async def _embed_query(client: genai.Client, text: str) -> np.ndarray | None:
    ...
    def _embed() -> list[float]:
        cfg = None
        if RAG_EMBED_TASK_TYPE:
            cfg = types.EmbedContentConfig(task_type=RAG_EMBED_TASK_TYPE)
...
async def _retrieve_rag_context_sql(query: str, client: genai.Client) -> str:
...
async def _retrieve_rag_context_memory(query: str, client: genai.Client) -> str:
    if not RAG_VECTOR_ENABLED:
        return ""
    ...
    for idx in idxs:
        ...
        items.append((text, meta, float(sims[idx])))
...
async def _retrieve_rag_context(query: str, client: genai.Client) -> str:
    if not RAG_VECTOR_ENABLED:
        return ""
    if _rag_use_sql():
        return await _retrieve_rag_context_sql(query, client)
    return await _retrieve_rag_context_memory(query, client)
...
async def chat(req: Request, payload: dict):
    ...
    if RAG_VECTOR_ENABLED:
        try:
            rag_context = await _retrieve_rag_context(user_text, client)
        except Exception as e:
